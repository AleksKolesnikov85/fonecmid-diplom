...
Функция СформироватьДвижения()
	...
	Запрос.Текст =
		...
		|	ВЫБОР
		|		КОГДА ВТ_Док.ВидРасчета = ЗНАЧЕНИЕ(ПланВидовРасчета.ВКМ_ОсновныеНачисления.Оклад)
		|			ТОГДА ЕСТЬNULL(ВКМ_УсловияОплатыСотрудниковСрезПоследних.Оклад, 0)
		|		КОГДА ВТ_Док.ВидРасчета = ЗНАЧЕНИЕ(ПланВидовРасчета.ВКМ_ОсновныеНачисления.ПроцентОтРабот)
		|			ТОГДА ЕСТЬNULL(ВКМ_УсловияОплатыСотрудниковСрезПоследних.ПроцентОтРабот, 0)
...
Процедура СформироватьДвиженияУдержания()
	...
	Запрос.Текст =
		...
		|	ВКМ_ОсновныеНачисления.ВидРасчета КАК БазовыйВидРасчета
		|ИЗ
...
Процедура РассчитатьПроцентОтРабот()
	...
	Запрос.Текст =
		...
		|ИЗ
		|	РегистрРасчета.ВКМ_ОсновныеНачисления.ДанныеГрафика(Регистратор = &Ссылка
		|	И ВидРасчета = &ПроцентОтРабот) КАК ВКМ_ОсновныеНачисленияДанныеГрафика
...
Процедура РассчитатьУдержания()
	...
	Запрос.Текст =
		...
		|		И ВКМ_Удержания.БазовыйВидРасчета = ВКМ_УдержанияБазаВКМ_ОсновныеНачисления.ВидРасчетаРазрез
		|ГДЕ
