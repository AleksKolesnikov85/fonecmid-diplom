...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если Не ЭтоНовый() Тогда
		...
		Если СпециалистДок <> Специалист Тогда
			Уведомление = СтрШаблон("В заявке №%1 выбран %2 в качестве исполнителя", Номер, Специалист);
			Уведомления.Добавить(Уведомление);
		КонецЕсли;
		
		Если ДатаПроведенияРабот <> ДатаПроведенияРабот Тогда
			Уведомление = СтрШаблон("В заявке №%1 изменилась дата проведения работ на %2", Номер, Формат(ДатаПроведенияРабот, "ДЛФ=D;"));
			Уведомления.Добавить(Уведомление);
		КонецЕсли;
		
		Если ВремяНачала <> ВремяНачалаРабот Тогда
			Уведомление = СтрШаблон("В заявке №%1 изменилось время начала работ на %2", Номер, Формат(ВремяНачалаРабот, "ДЛФ=T;"));
			Уведомления.Добавить(Уведомление);
		КонецЕсли;
		
		Если ВремяОкончания <> ВремяОкончанияРабот Тогда
			Уведомление = СтрШаблон("В заявке №%1 изменилось время окончания работ на %2", Номер, Формат(ВремяОкончанияРабот, "ДЛФ=T"));
			Уведомления.Добавить(Уведомление);
		КонецЕсли;
